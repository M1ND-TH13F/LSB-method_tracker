...
def lsb_hide_method(image, payload_data, image_number):
    image_dir = "../../Desktop/проект/empty_s"
...
for i in range(len(empty_c)):
    if empty_c[i] < full_c[i]:
        print(f"stego_full{i}.png - заполненный контейнер")
        count_full+=1
    elif empty_c[i] > full_c[i]:
        print(f"stego_full{i}.png - пустой контейнер")
print(f"Итого заполненных контейнеров: {count_full}")